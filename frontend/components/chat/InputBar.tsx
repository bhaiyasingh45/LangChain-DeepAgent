...
export function InputBar() {
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLTextAreaElement>) => {
    if (suggestions.length > 0) {
      ...
      if (e.key === "Tab" || (e.key === "Enter" && !e.shiftKey && suggestions.length > 0 && !isExactCommand)) {
        e.preventDefault();
  ...
  return (
    <div className="border-t border-cc-border bg-cc-surface px-3 py-2 relative">
      ...
      <div className="flex items-end gap-2">
        {/* Attach button */}
        <button
          className="text-cc-dim hover:text-cc-text transition-colors pb-1.5 flex-shrink-0"
          title="Attach file"
          disabled={streaming}
        >
        ...
        <textarea
          ...
          placeholder={streaming ? "Agent is running..." : "Ask anything... (/ for commands)"}
          disabled={streaming}
          rows={1}
